document.addEventListener('DOMContentLoaded', function() {
    ...
    generateAndShareImageBtn.addEventListener('click', async function() {
        try {
            ...
            const canvas = await html2canvas(elementToCapture, { 
                useCORS: true,
                scale: 1.5, // Mantém boa qualidade da imagem (500px base * 1.5 = 750px largura final)
                logging: false,
                onclone: (clonedDoc) => {
    ...
    function generateReportVisualContentHtml(eventName, data, total, generationDate, filterZerosForImage) {
        ...
        return `
            ...
            <div class="container" style="width:100%; max-width:100%; margin:0; padding:0; box-shadow:none; border-radius:0; background-color: white;"> 
                ...
                <div class="report-output-container" style="padding: 20px 15px;"> {/* Padding ajustado para container menor */}
    ...
    function getReportStylesForCanvas() {
        // Para o HTML do relatório baixado, usamos os estilos originais (mais espaçosos)
        // Para o canvas, usamos estilos um pouco mais compactos.
        return getReportStyles(true); // true indica que é para o canvas (compacto)
    }

    function getReportStylesForHtmlDownload() {
        return getReportStyles(false); // false indica que é para HTML (padrão)
    }
    
    // Novo parâmetro `isForCanvas` para diferenciar estilos
    function getReportStyles(isForCanvas = false) {
        // Ajustes de estilo baseados se é para o canvas (imagem mais vertical/compacta)
        const headerH1Size = isForCanvas ? '1.6em' : '1.8em';
        const tableSectionH2Size = isForCanvas ? '1.3em' : '1.4em';
        const tableCellPadding = isForCanvas ? '10px 8px' : '12px 15px';
        const tableCellFontSize = isForCanvas ? '0.9em' : '0.95em';
        const reportOutputPadding = isForCanvas ? '20px 15px' : '25px 30px'; // Usado no HTML gerado para download
        ...
        <style>
            ...
            * { 
            ...
            body { 
                ...
                line-height: 1.5; /* Ajustado para compactar um pouco mais se necessário */
            ...
            /* Este .report-output-container é para o HTML de download, o do canvas é ajustado inline */
    ...
    function generateFullHtmlReportString(eventName, data, total, generationDate) {
        const styles = getReportStylesForHtmlDownload(); // Usa os estilos padrão/mais espaçosos
        ...
        return `
            ...
            <body>
                <div class="container" style="max-width: 800px; margin: 20px auto; box-shadow: 0 6px 12px rgba(0,0,0,0.1); border-radius: 8px;">
                    ...
                    <div class="report-output-container"> {/* Estilo de padding virá do getReportStyles(false) */}
                        <section class="table-section">
                            <h2>${escapeHTML(eventName)}</h2>
                            <table>
                                <thead>
